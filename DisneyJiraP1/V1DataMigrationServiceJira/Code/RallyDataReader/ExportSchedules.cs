...
namespace RallyDataReader
{
    public class ExportSchedules : IExportAssets
    {
         ...
         * NOTES:
         * 
         * - For Rally migrations, we must create the schedules in V1 as they do not exist Rally.
         * - Schedules are created for all subprojects then applied recursively to each subproject's releases.
         * - The schedules are dervied from calcuating the gap and length from the release's first iteration.
         * 
         *****************************************************************************************************************************************/
        ...
        private void SetProjectSchedules(string ParentOID)
        {
            ...
            while (sdr.Read())
            {
                ...
                {
                    scheduleCount++;
                    CreateProjectSchedule(sdr["Name"].ToString(), scheduleCount);
