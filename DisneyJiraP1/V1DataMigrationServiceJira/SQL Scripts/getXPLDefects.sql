...
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'getXPLDefects')
    DROP PROCEDURE [dbo].getXPLDefects;
GO

CREATE PROCEDURE getXPLDefects
AS
BEGIN
	SET NOCOUNT ON;
...
--Remove any asset that is not a defect.
...
select a.id as 'defectId',
	   a.name as 'defectName',
	   a.priority,
	   a.topic,
	   a.description,
	   a.value,
